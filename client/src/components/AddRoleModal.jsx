...
import { useCreateRoleMutation } from "../redux/slices/api/roleApiSlice";
import { toast } from "sonner";
import styles from '../pages/styles/RoleModal.module.css';

const AddRoleModal = ({ open, setOpen, branchId, refetch }) => {
  ...
  const [permissions, setPermissions] = useState([
    ...
    { name: "can view organisation", value: false },
    ...
    { name: "can modify team list", value: false },
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      //console.log("Submitting role creation with data:", { name, permissions: selectedPermissions, description, branchId });
      await createRole({ name, permissions: selectedPermissions, description, branchId }).unwrap();
  ...
  return (
    <ModalWrapper open={open} setOpen={setOpen} title="Add New Role" className={`w-3/4 max-w-4xl ${styles.modal}`}>
                ...
                {permissions.slice(Math.ceil(permissions.length / 2)).map((permission, index) => (
                  <div key={permission.name} className="flex items-center">
                    <input
                      ...
                      type="checkbox"
                      checked={permission.value}
                      onChange={() => handlePermissionChange(Math.ceil(permissions.length / 2) + index)}
                      className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                    />
                    <label htmlFor={permission.name} className="ml-2 text-sm text-gray-600">
                      {permission.name}
