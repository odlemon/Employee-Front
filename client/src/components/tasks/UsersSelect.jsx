...
export default function UserList({ team, setTeam, branch }) {
  ...
  useEffect(() => {
    if (data) {
      const filtered = data.filter(user => user.branch === branch);
      setFilteredUsers(filtered);

      if (team?.length < 1 && filtered.length > 0) {
        setSelectedUsers([filtered[0]]);
      } else {
        setSelectedUsers(team?.filter(user => user.branch === branch) || []);
      }
    }
  }, [data, branch, team]);
  ...
  return (
    <div className=''>
      <p className='text-slate-900 dark:text-gray-500'>Assign Objective To:</p>
      <Listbox
        value={selectedUsers}
